...
public interface SortingRule<T> {
    ...
    if (Configs.instance.enableSorting && Configs.instance.sortingInfluenceRange == SortingInfluenceRange.INVENTORY_ONLY) {
      if (Configs.instance.sortingCalculationType != SortingCalculationType.STANDARD || Internal.cachedInventoryItems == null) {
        ...
        if (Configs.instance.sortingCalculationType == SortingCalculationType.STANDARD) {
          ...
          final List<Item> list = stream.collect(Collectors.toList());
  ...
  class SortingRuleContainer<T> {
     ...
     */
    public SortingRuleContainer(@NotNull SortingRule<T> sortingRule, int priority, @Nullable @NonNls String name) {
    ...
    public boolean equals(Object o) {
      ...
      if (!(o instanceof SortingRuleContainer)) return false;

      SortingRuleContainer<?> that = (SortingRuleContainer<?>) o;

      if (priority != that.priority) return false;
      if (!sortingRule.equals(that.sortingRule)) return false;
      return Objects.equals(name, that.name);
    }

    @Override
    public int hashCode() {
      int result = sortingRule.hashCode();
