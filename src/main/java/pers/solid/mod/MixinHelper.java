...
public class MixinHelper implements ModInitializer {
  ...
  public static <T> void applyItemWithFollowings(
      ...
      T item,
      Consumer<T> action,
      Consumer<T> actionForFollowings) {
    action.accept(item);
   ...
   */
  public static <T> boolean isCombinationFollower(
      T item, Collection<? extends Map<T, ? extends Collection<T>>> rules) {
    for (Map<T, ? extends Collection<T>> rule : rules) {
  ...
  /**
   * 将所有的物品组转移规则编译到 {@link #COMPILED_ITEM_GROUP_TRANSFER_RULES}
   * 中以供使用。之所以编译是为了加快读取速度。在修改配置时，也会对物品组进行重新编译。<br>
   * 但是，考虑到第三方模组也会修改配置，而其 {@code saveConsumer} 不会触发重新编译，所以这里也暂时先不重新编译。
   *
   * @param itemGroupTransferRules 物品组转换规则，是由物品到物品组的映射的列表。
   */
  @Deprecated
  public static void compileItemGroupTransferRules(
      ObjectList<Map<Item, ItemGroup>> itemGroupTransferRules) {
    ...
    for (Item item : Registry.ITEM) {
      ...
      if (!build.isEmpty()) COMPILED_ITEM_GROUP_TRANSFER_RULES.put(item, build);
    }
  }
  ...
  public void onInitialize() {
    ...
    Configs.updateVariantsFollowingBaseBlocks(
        configs.variantsFollowingBaseBlocks, BlockFamilyRule.AFFECTED_VARIANTS);
    ...
    Configs.updateCustomRegexTransferRules(
        configs.regexTransferRules, Configs.CUSTOM_REGEX_TRANSFER_RULE);
