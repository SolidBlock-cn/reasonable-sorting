...
public class DefaultItemSortingRuleSupplier implements Supplier<@Unmodifiable Collection<@Unmodifiable Map<Item, @Unmodifiable Collection<Item>>>> {
  /**
   * 此模组默认内置的物品组合规则。
   */
  public static final @Unmodifiable ImmutableMap<Item, @Unmodifiable Collection<Item>> DEFAULT_ITEM_SORTING_RULE = new ImmutableMap.Builder<Item, Collection<Item>>()
      ...
      .put(Items.FLINT, ImmutableList.of(Items.SNOWBALL, Items.LEATHER))
      .build();
  ...
  public @Unmodifiable Collection<@Unmodifiable Map<Item, @Unmodifiable Collection<Item>>> get() {
    return Collections.singleton(new ForwardingMap<>() {
      @Override
      protected @NotNull @Unmodifiable Map<Item, @Unmodifiable Collection<Item>> delegate() {
